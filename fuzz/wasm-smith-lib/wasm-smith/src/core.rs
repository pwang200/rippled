...
impl Module {
    ...
    fn add_arbitrary_export(&mut self, name: String, kind: ExportKind, idx: u32) -> Result<()> {
        let idx = if name == "fuzz" && kind == ExportKind::Func {
            self.ensure_named_one_func()?
        } else {
            idx
        };
    ...
    fn ensure_named_one_func(&mut self) -> Result<u32> {
        fn matches_named_one(ty: &FuncType) -> bool {
            ty.params.is_empty() && ty.results.as_slice() == [ValType::I32]
        }
        ...
        if let Some((idx, _)) = self
            ...
            .iter()
            .enumerate()
            .find(|(_, (_, ty))| matches_named_one(ty))
        {
        ...
        for idx in self.func_types.iter().copied() {
            let ty = self.func_type(idx);
            if matches_named_one(ty) {
                type_index = Some(idx);
                break;
            }
        }
